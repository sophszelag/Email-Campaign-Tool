...
export type CampaignFields = {
  ...
  /** Free text like "Hockey, lacrosse, baseball, softball". Row is omitted when unset. */
...
// sized for the green header band.
 ...
 * single recipient. Markup/copy/CSS here is taken directly from
 * sidelineswap-monkeysports-woodbridge-invite.html (the real mockup),
 * with the Woodbridge-specific details swapped for merge fields.
 *
 * Known gaps vs. that mockup, called out so they don't get "fixed" by
 * accident later:
 *  - The primary/secondary CTAs ("Reserve your spot", "Find another event
 *    near you") still point at "#" — the mockup didn't specify a real
 *    destination and Phase 1 has no RSVP/event-finder page to send them
 *    to. Needs a real URL (or removal) before a live send.
 *  - The mockup's footer also has "Update preferences" and "View in
 *    browser" links; both are dropped here since neither a preference
 *    center nor a web-view route exists yet, and shipping dead links in a
 *    real customer email is worse than omitting them. Unsubscribe is the
 *    one footer link that's fully real (signed, working token).
 *  - This uses <style> + classes like the source file, not inlined CSS.
 *    That's fine for Gmail/Apple Mail; Outlook desktop's rendering engine
 *    strips <style> blocks and will show a degraded (but still readable)
 *    version. Worth running through an inliner (e.g. MJML, Juice) before
 *    a large send if Outlook desktop opens matter for this audience.
 */
export function renderInviteEmail(
  ...
  const cityState = campaign.event_state ? `${campaign.event_city}, ${campaign.event_state}` : campaign.event_city;
  const whenLine = campaign.event_hours ? `${campaign.event_dates} · ${campaign.event_hours}` : campaign.event_dates;

  const subject = `We're back in ${campaign.event_city}.`;
  const preheader = `${firstName} — trade in gear at ${campaign.event_venue} ${campaign.event_city} ${campaign.event_dates}, plus a bonus for coming back`;
...
<style>
  * { box-sizing: border-box; }
  ...
  .email {
    ...
    margin: 0 auto;
    background: #FFFFFF;
    box-shadow: 0 4px 20px rgba(37, 60, 50, 0.08);
    overflow: hidden;
  }
  .preheader {
    background: #F1EFE8;
  ...
  .hero-headline .accent { color: #02C874; }
  .hero-sub {
    font-family: 'Roboto', sans-serif;
    font-weight: 500;
    font-size: 15px;
  ...
  .body-copy strong { font-weight: 700; color: #253C32; }
  .body-copy .payout { font-weight: 900; color: #02C874; font-size: 18px; }
  .details { background: #CCDCD4; padding: 22px 24px; margin: 24px 0 20px; border-radius: 4px; }
  .details-eyebrow {
    font-family: 'Roboto', sans-serif;
    font-weight: 700;
    font-size: 11px;
  ...
  .details-row { font-family: 'Roboto', sans-serif; font-size: 15px; color: #253C32; padding: 6px 0; line-height: 1.5; }
  .details-row .label { font-weight: 700; display: inline-block; min-width: 82px; }
  .guidelines { background: #FFFFFF; border: 2px solid #CCDCD4; padding: 20px 24px; margin: 8px 0 28px; border-radius: 4px; }
  .guidelines-eyebrow {
    font-family: 'Roboto', sans-serif;
    font-weight: 700;
    font-size: 11px;
  ...
  .guidelines-intro { font-family: 'Roboto', sans-serif; font-weight: 500; font-size: 15px; color: #253C32; margin: 0 0 10px; line-height: 1.5; }
  .guidelines-list { margin: 0; padding: 0; list-style: none; }
  .guidelines-list li { font-family: 'Roboto', sans-serif; font-size: 15px; color: #171B1F; padding: 5px 0 5px 24px; line-height: 1.5; position: relative; }
  .guidelines-list li::before { content: "✓"; position: absolute; left: 0; top: 4px; color: #02C874; font-weight: 900; font-size: 15px; }
  .bonus { background: #02C874; padding: 26px 24px; text-align: center; margin: 8px 0 32px; border-radius: 4px; }
  .bonus-eyebrow { font-family: 'Roboto', sans-serif; font-weight: 700; font-size: 11px; letter-spacing: 0.14em; color: #253C32; text-transform: uppercase; margin: 0 0 8px; }
  .bonus-code { font-family: 'Roboto', sans-serif; font-weight: 900; font-size: 34px; letter-spacing: 0.08em; color: #253C32; margin: 0 0 4px; }
  .bonus-sub { font-family: 'Roboto', sans-serif; font-weight: 500; font-size: 13px; color: #253C32; margin: 0; opacity: 0.75; }
  ...
  .divider { height: 1px; background: #E5E7E4; margin: 32px 40px 0; }
  .tagline-block { padding: 28px 40px 24px; text-align: center; }
  .tagline { font-family: 'Roboto', sans-serif; font-weight: 500; font-style: italic; font-size: 14px; color: #253C32; margin: 0; letter-spacing: 0.01em; }
  .footer { background: #253C32; padding: 28px 40px; text-align: center; }
  .footer-copy { font-family: 'Roboto', sans-serif; font-weight: 400; font-size: 12px; line-height: 1.7; color: #CCDCD4; margin: 0 0 10px; }
  .footer-links a { color: #FFFFFF; text-decoration: underline; font-size: 12px; font-family: 'Roboto', sans-serif; margin: 0 6px; }
  @media (max-width: 620px) {
    body { padding: 16px 0; }
    .preheader { padding: 10px 20px; }
    ...
    .bonus-code { font-size: 26px; letter-spacing: 0.06em; }
    .details, .guidelines, .bonus { padding-left: 18px; padding-right: 18px; }
  }
</style>
</head>
<body>
  ...
  <div class="email">
    ...
    <div class="hero">
      <p class="eyebrow">Trade-in event · ${escapeHtml(cityState)}</p>
      <h1 class="hero-headline">We're back in <span class="accent">${escapeHtml(campaign.event_city)}</span>.</h1>
      <p class="hero-sub">${escapeHtml(whenLine)}</p>
    </div>
    <div class="body">
      <p class="greeting">Hey ${escapeHtml(firstName)},</p>
      <p class="body-copy">Last time we rolled through your area, you traded in <span class="payout">${payout}</span> worth of gear with us. On <strong>${escapeHtml(campaign.event_dates)}</strong>, we're setting up at <strong>${escapeHtml(campaign.event_venue)} in ${escapeHtml(campaign.event_city)}</strong> — and we'd love to see you again.</p>
      <p class="body-copy">Same drill as before: bring the gear you're not using, our team appraises it on the spot, and you walk out with store credit. No listings, no shipping, no waiting.</p>
      <div class="details">
        <p class="details-eyebrow">Event details</p>
        <div class="details-row"><span class="label">When</span> ${escapeHtml(whenLine)}</div>
        <div class="details-row"><span class="label">Where</span> ${escapeHtml(campaign.event_venue)} · ${escapeHtml(cityState)}</div>
        ${campaign.accepted_categories ? `<div class="details-row"><span class="label">Bringing</span> ${escapeHtml(campaign.accepted_categories)}</div>` : ""}
      </div>
      <div class="guidelines">
        <p class="guidelines-eyebrow">Before you pack up</p>
        <p class="guidelines-intro">We'll take gear that checks these boxes:</p>
        <ul class="guidelines-list">
          <li>5–7 years old or newer</li>
          <li>Retailed for $100+ when new</li>
          <li>In working condition, not broken or damaged</li>
        </ul>
      </div>
      <p class="body-copy">And because you traded with us before, we're adding a bonus on top of your store credit this time. Show this code at check-in:</p>
      <div class="bonus">
        <p class="bonus-eyebrow">Your bonus code</p>
        <p class="bonus-code">${escapeHtml(campaign.bonus_code)}</p>
        <p class="bonus-sub">Valid ${escapeHtml(campaign.event_dates)} only · ${escapeHtml(campaign.event_city)} event</p>
      </div>
      <div class="cta-wrap">
        <a href="#" class="cta">Reserve your spot →</a>
        <a href="#" class="cta-secondary">Can't make it? Find another event near you</a>
      </div>
  ...
  const detailsLines = [`When: ${whenLine}`, `Where: ${campaign.event_venue} · ${cityState}`];
  if (campaign.accepted_categories) {
    detailsLines.push(`Bringing: ${campaign.accepted_categories}`);
  }
  ...
  const text = [
    ...
    `Hey ${firstName},`,
    "",
    `Last time we rolled through your area, you traded in ${payout} worth of gear with us. On ${campaign.event_dates}, we're setting up at ${campaign.event_venue} in ${campaign.event_city} — and we'd love to see you again.`,
    "",
    "Same drill as before: bring the gear you're not using, our team appraises it on the spot, and you walk out with store credit. No listings, no shipping, no waiting.",
    "",
    "Event details",
    ...detailsLines,
    "",
    "Before you pack up, we'll take gear that's 5-7 years old or newer, retailed for $100+ when new, and in working condition.",
    "",
    `Your bonus code: ${campaign.bonus_code} (valid ${campaign.event_dates} only, ${campaign.event_city} event)`,
    "",
